...
public class ListContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        Level level;
        if(tokens.size()==1)    level=null;
        else if(tokens.get(1)=="HIGH")   level = Level.HIGH;
        else if(tokens.get(1)=="MEDIUM")   level = Level.MEDIUM;
        else if(tokens.get(1)=="LOW") level = Level.LOW;
        else    level = null;
